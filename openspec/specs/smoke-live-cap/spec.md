# Capability: Smoke Live (WebSocket) Cap (smoke-live-cap)

## Purpose

`scripts/smoke_1_0.py` 的 IM（WebSocket）域：默认安全关闭（不建立公网连接）；启用时显式 opt-in，超时/流控统一 SKIP 不 FAIL；全程不触发任何写操作（发消息/撤回/建会话）。
