# Capability: Smoke HTTP Cap (smoke-http-cap)

## Purpose

提供 `scripts/smoke_1_0.py` 端到端冒烟基线的 HTTP 域：无凭证自动 mock 保证结构校验；有凭证真实调用，对写接口（擦亮/下单）做宽松判定（防止误下订单）。统一退出码 0/2/3 与 `--json` 凭证脱敏。
